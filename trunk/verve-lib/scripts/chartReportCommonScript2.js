loadScript(root_folder + "/scripts/utils/pentaho_chart.js");
...
function initChartPageTemplate(settings){
	settings=$.extend({},{
						...
						generateChartSubTitle: function(domContentForm){return ''},
	...
	var generateControlPanles=function(){
		...
		initControlPanel({
			...
			success: function(jsonData){
				generateChart({						
						divResult: divResult,
						jsonData: jsonData,	
						chartTitle: settings.reportName,
						miniMode: false,
						chartSubTitle: settings.generateChartSubTitle(domContentForm0) //can be string or an function
						});
			}
		});	
		//generate left chart
		...
		initControlPanel({
			...
			success: function(jsonData){
				generateChart({						
						...
						chartTitle: settings.reportName,
						miniMode: true,
						chartSubTitle: settings.generateChartSubTitle(domContentForm1) //can be string or an function
						});
			}
		});
		...
		initControlPanel({
			...
			success: function(jsonData){
				generateChart({						
						divResult: divResult2,
						jsonData: jsonData,	
						chartTitle: settings.reportName,						
						miniMode: true,
						chartSubTitle: settings.generateChartSubTitle(domContentForm2) //can be string or an function
						});
			}
		});	
	}
