...
var methods = {
  ...
  reprocess_display : function(options){
	...
	if (options.result_table     === undefined) options.result_table    = $('$results');
	...
	//chinh
	//alert(options.result_table.attr('name'));
	
    methods.populate_containers();
    methods.update_results(options.result_table);
  ...
  update_results : function(p_result_table){
	if (p_result_table     === undefined) p_result_table    =  $('#results');
    ...
    var result_table = p_result_table,
        result_rows;
    result_table.empty();

    snip += '<table name="pivot-table" class="table table-striped table-condensed"><thead>';
    ...
    snip += '</thead></tr><tbody name="result-rows"></tbody></table>';
    result_table.append(snip);

    result_rows = result_table.find('tbody[name=result-rows]');
