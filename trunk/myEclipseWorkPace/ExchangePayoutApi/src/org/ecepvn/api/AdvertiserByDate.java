...
public class AdvertiserByDate extends MainApi{

	public AdvertiserByDate() {
		super();
		// TODO Auto-generated constructor stub
		this.setDataSourceTableName("agg_creatives_by_date");
	...
	public ApiResponseResultSetInfo getInfo(HttpServletRequest request) {
		...
		String apiUrl = Configure.getConfig("advertiserByDateAPIUrl");
		...
		info.addSelectExample("GET " + rootUrl + "?select=date|flight_id");
		// add by example
		info.addByExample("GET " + rootUrl + "?by=clicks");
		info.addByExample("GET " + rootUrl + "?by=clicks|impressions");
		info.addByExample("GET " + rootUrl + "?select=date|flight_id&by=impressions");
		// add where example
		...
		info.addWhereExample("GET " + rootUrl + "?where[date]="	+ currentDateExample + "&where[flight_id.in]=24067710,24068670,25114350");
		info.addWhereExample("GET " + rootUrl + "?select=date&by=impressions&where[date]="	+ currentDateExample);
		// add order example
		info.addOrderExample("GET " + rootUrl + "?order=date.desc");
		info.addOrderExample("GET " + rootUrl + "?order=date.desc");
		info.addOrderExample("GET " + rootUrl + "?select=date&by=impressions&where[date]="	+ currentDateExample + "&order=date.desc");
		return info;
	}
