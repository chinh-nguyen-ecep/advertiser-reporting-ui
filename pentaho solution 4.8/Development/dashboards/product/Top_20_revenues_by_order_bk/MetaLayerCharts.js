...
MetaLayerCharts =  {
	...
	topSitesOfYesterdayChartDefinition:{
	    colHeaders: ["Order name","Revenue","Impressions","Clicks","BBudget","ABudget","Pacing","MTDRevenue","RMBudget","NMBudget","TDRank","YRank"],
	    ...
	    var query = "select NON EMPTY {[Measures].[Revenue],[Measures].[Impressions], [Measures].[Clicks], [Measures].[Budget],[Measures].[ABudget], [Measures].[Pacing], [Measures].[MTD Total Revenue], [Measures].[Remaining Monthly Budget], [Measures].[Next Month Budget], [Measures].[Rank], [Measures].[Yesterday Rank]} ON COLUMNS," +
	...
	topSitesOf1DayAgoChartDefinition:{
			colHeaders: ["Order name","Revenue","Impressions","Clicks","BBudget","ABudget","Pacing","MTDRevenue","RMBudget","NMBudget","YRank","TDRank"],
			colTypes: ['string','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric'],
			colFormats: [null,'%.0f','%.0f','%.0f','%.0f','%.0f','%.0f','%.0f','%.0f','%.0f','%.0f','%.0f'],
			colWidths: ['300px','100px',null,null,null,null,null,null,null,null,null,null],
		    queryType: 'mdx',
		    ...
		    var query = "select NON EMPTY {[Measures].[Revenue],[Measures].[Impressions], [Measures].[Clicks], [Measures].[Budget],[Measures].[ABudget], [Measures].[Pacing], [Measures].[MTD Total Revenue], [Measures].[Remaining Monthly Budget], [Measures].[Next Month Budget], [Measures].[Rank], [Measures].[Rank After One Day]} ON COLUMNS," +
		    		"  NON EMPTY {[Order name].[All Order names].Children} ON ROWS " +
		    		"from [daily_agg_revenue_by_order] " +
		    		"where Crossjoin({[Full date].["+oneDayAgo+"]}, {[Is active].[true]})";
		    return query;
		    }
	},
