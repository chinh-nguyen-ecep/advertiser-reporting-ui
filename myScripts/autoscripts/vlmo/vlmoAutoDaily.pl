...
$group_process_name="VLMO";	#the name of group process to register to daily process status table

$locatefd="/opt/temp/autoscripts/vlmo";	#the location content this script
$logFile="$locatefd/logs/$report_date.txt";	#the location cotent log file
my $emailErrorTitle="[Error!][Daily VLMO report][$report_date]";	#the email title used when send a mail error 
my $emailAvailableTitle="[Notification!][Daily VLMO report][$report_date]";	#the email title used when send a mail
...
sub main{
	...
	if($error == 0){
		...
		runParam(74);
		dw3_writelog($logFile,"transfer daily VLMO to staging environment");
		
		#transfer daily VLMO to production environment
		runParam(75);
		...
		dw3_sendMail($mailto,$emailAvailableTitle,"Dear all,<p />Daily VLMO $report_date has finished with no error.<p />Process detail:<p/>$log<p />Thanks,<br />\-\-\-send by auto mail.");
...
sub checkSU{
	...
	while($repeat eq "y"){
		...
		while($totalFiles<1){
			...
			#Cretae String query		
			my $query=" select count (*) AS Total_Files from control.data_file WHERE data_file_config_id = ? AND file_timestamp::date=?;";
			my $query_handle = $dbh->prepare($query);
			$query_handle->execute(188,$report_date2);
		...
		$query="SELECT file_name,file_status,staging_load_count,fact_table_load_count FROM control.data_file WHERE data_file_config_id = ? AND file_timestamp::date=?";
		...
		if($sendalertmail==0){
			...
			"Dear all,<p />
			Daily VLMO report $report_date on dw3 begin start with total files today is $totalFiles and now Su files is $SU file.<p />
			<p />Thanks,<br />\-\-\-send by auto mail.");
			}else{
			dw3_sendMail($mailto,$emailAvailableTitle,
			"Dear all,<p />
			Daily VLMO report $report_date on dw3 begin start with total files today is $totalFiles and now Su files is $SU file.<p />Thanks,<br />\-\-\-send by auto mail.");
			};
		}
