...
while($repeat eq "y"){
	...
	if($hour==12 && $min==59){
		...
		system("cd $rootFD && perl main.pl dim dw3 dw10 scheduleTransferDim.pl");
		system("cd $rootFD && perl main.pl dim dw10 dw10:analyticsdb scheduleTransferDim.pl");
		system("cd $rootFD && perl main.pl dim dw10 pentaho.ecepvn.org:analyticsdb scheduleTransferDim.pl");
	}
	sleep(50);
}
