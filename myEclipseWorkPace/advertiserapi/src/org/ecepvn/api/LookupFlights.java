...
public class LookupFlights extends MainApiLookup{

	public LookupFlights() {
		super();
		// TODO Auto-generated constructor stub
		this.setDataSourceTableName("dim_flights");
		this.setDefaultDimensions(new String[]{"dfp_flight_id"});
	}
	@Override
	public ApiResponseResultSetInfo getInfo(HttpServletRequest request) {
		...
		String apiUrl = Configure.getConfig("lookupFlightsUrl");
		...
		info.addDimension(new String[] { "adm_billing_line_item_id", "", "int" });
		info.addDimension(new String[] { "adm_order_id", "", "int" });
		info.addDimension(new String[] { "adm_flight_name", "", "string" });
		...
		// add constraint
		info.addConstraint(new String[] { "dfp_flight_id.in", "integer","1,2,3" });
		...
		info.addSelectExample("GET " + rootUrl + "?select=dfp_flight_id|adm_flight_id|adm_order_id");
		// add where example
