...
MetaLayerCharts =  {
	...
	topSitesOfYesterdayChartDefinition:{
	    colHeaders: ["Order name","BBudget","ABudget","Revenue","Impressions","Clicks","Pacing","MTDRevenue","RMBudget","NMBudget","TDRank","YRank"],
	    ...
	    var query = "select NON EMPTY {[Measures].[Budget],[Measures].[ABudget],[Measures].[Revenue],[Measures].[Impressions], [Measures].[Clicks], [Measures].[Pacing], [Measures].[MTD Total Revenue], [Measures].[Remaining Monthly Budget], [Measures].[Next Month Budget], [Measures].[Rank], [Measures].[Yesterday Rank]} ON COLUMNS," +
	...
	topSitesOf1DayAgoChartDefinition:{
			colHeaders: ["Order name","BBudget","ABudget","Revenue","Impressions","Clicks","Pacing","MTDRevenue","RMBudget","NMBudget","YRank","TDRank"],
			colTypes: ['string','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric'],
			colFormats: [null,'%.0f','%.0f','%.0f','%.0f','%.0f','%.0f','%.0f','%.0f','%.0f','%.0f','%.0f'],
			colWidths: ['300px','100px',null,null,null,null,null,null,null,null,null,null],
		    queryType: 'mdx',
		    ...
		    var query = "select NON EMPTY {[Measures].[Budget],[Measures].[ABudget],[Measures].[Revenue],[Measures].[Impressions], [Measures].[Clicks],[Measures].[Pacing], [Measures].[MTD Total Revenue], [Measures].[Remaining Monthly Budget], [Measures].[Next Month Budget], [Measures].[Yesterday Rank], [Measures].[TDRank]} ON COLUMNS," +
		    		"  NON EMPTY {[Order name].[All Order names].Children} ON ROWS " +
